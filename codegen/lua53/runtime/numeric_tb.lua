...
local function patch(fn)
	return function(a,b)
		a = math.floor(a)
		b = math.floor(b or 0xFFFFFFFF)
		return fn(a,b)
	end
end
Numeric.PATCH = patch
local bit_and = patch(bit32.band)
local bit_or = patch(bit32.bor)
local bit_xor = patch(bit32.bxor)
local bit_not = bit32.bnot

local bit_extract = bit32.extract
local bit_replace = bit32.replace

local table_freeze = function(x) return x end
...
function Numeric.into_u32(data)
	return bit_rshift(data,32), bit_and(data)
end
