...
fn write_export_of(list: &[Export], wanted: External, w: &mut dyn Write) -> Result<()> {
	...
	writeln!(w, "\t\t{lower} = {{")?;
	for Export { name, index, .. } in list.iter().filter(
		|v| (External::from(v.kind) == wanted)
	) {
		write!(w, "\t\t\t")?;
...
fn write_local_operation(head: &str, tail: &str, w: &mut dyn Write) -> Result<()> {
	...
	match (head, tail) {
		...
		("band" | "bor" | "bxor" | "bnot", _) => write!(w, "Integer.PATCH(bit32.{head})"),
		("shl", _) => write!(w, "bit32.lshift"),
		("shr", "i32" | "i64") => write!(w, "bit32.arshift"),
		("shr", "u32" | "u64") => write!(w, "bit32.rshift"),
		("rotl", _) => write!(w, "bit32.rol"),
		("rotr", _) => write!(w, "bit32.ror"),
...
pub fn from_module_typed(wasm: &Module, type_info: &TypeInfo, w: &mut dyn Write) -> Result<()> {
	...
	writeln!(w, "local table_new = function(...) return {{}} end")?;
